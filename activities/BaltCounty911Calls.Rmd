...
The data all seems to stick within this 8-13ish range. I'd draw lines at 17 and at 4 or 5.
```{r daily-calls-plot}
# Plot daily call volume
ggplot(daily_calls, aes(x = date, y = call_count)) +
  geom_line(color = "hotpink", alpha = 0.7) +
  geom_point(color = "hotpink", alpha = 0.5, size = 1) +
  theme_minimal() +
...
I was just a bit off, now the lines fall at 3 and 18. 
...
I would describe Saturday as the day with the greatest (positive) deviation from the average number of overdose calls. I[d further explain it as having the largest addition of overdose calls to the daily average in Baltimore County.
...
ggplot(daily_calls, aes(x = date)) +
  ...
  geom_line(aes(y = rolling_avg_7day), color = "hotpink", size = 1) +
  theme_minimal() +
...
I may look more into the August jump in overdose calls or take the holidays route. 4/20 and New Years' Day stuck out to me as dates with high overdose calls. I also may look into a trend with weekends since many of them seem to be on Friday, Saturday, and Sunday. I have a feeling a lot of these calls are happening at all hours of the morning so that would probably be the angle I land on. I would try to get more data about the times of these calls because I think that's where the story really is since there isn't too obvious of a trend. I would tell my editor that the weekend trend is there to start but I think there's a deeper one with the timing.
