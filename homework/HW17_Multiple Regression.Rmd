---
title: "HW17_Multiple Regression"
...
Income, Pct Bachelors Higher, and Median Age all seem very correlated to each other. There is a positve correlation between them and voter turnout and well as for Pct White. Population and Pct Unemployed are negatively correlated with voter turnout, though the correlation for population may not be significant or linear. They may also be correlated with each other.
...
All of the coefficients are extremely small which implies no relationship between the independent variables and the dependent variable. However, since the residuals are fairly close to 0 and the p-value is way less than 0.05, the model is probably significant if some of the variables are removed. If you look at the starred rows, they have p-values below 0.05 and should be the significant variables to this model. The model is useful if not overloaded with data.
...
The three variables alone were not super significant but all three of them together was very significant. The models sort themselves by significance depending on Adjusted R-squared values in a way that also reflects the number of significant independent variables to them. It almost makes sense that the outcome of the model with income, median age and percent White is the same as the full model because they are the onyl significant variables in the full model.
...
# Model 4
model4 <- lm(turnout ~ pct_white + income, data = md_turnout)
summary(model4)
...
I would explain that voter participation is dependent on a variety of factors together: income, median age, and racial composition. After sifting through the data, we found that these three factors together had the most significant impact on voter turnout. You could then apply that to respective cities based on their actual demographic data and even take it a step further to maybe predict voter turnout in specific cities.
