...
Yes, because all of the scores are greater than the prior mean, which means that the current mean will be higher and the test scores have improved.
...
The mean has increased by 3-ish points. The standard deviation has also decreased a lot--by more than half, which means that the scores are closer together. I wonder why the scores have increased, and what is causing the scores to be closer together. I'm almost thinking that because the scores are so close together, that means that there is a failure on the district's part to cover certain topics in the necessary depth.
...
my_fill_color <- "hotpink" # Replace with a color name like "royalblue", "darkgreen", etc.
...
ggplot(aes(x = classroom, y = reading_score)) +
  ...
  geom_hline(yintercept = prior_mean, color = "pink", size = 1, linetype = "dashed") +
  annotate("text", x = 2, y = prior_mean - 1, 
           label = "Previous Average (72.6)", hjust = 0, fontface = "bold", color = "pink") +
  labs(
...
Reading scores compared to previous average
...
**Hypotheses:**
Null: The new reading program did not improve reading test scores
Alternative: The new reading program did improve reading test scores
...
The p-value is MUCH smaller than 0.05 which means that the null hypothesis is rejected. This confirms the alternative hypothesis that the new reading program did improve reading test scores. I would explain this by saying that the probability of the new program NOT improving the test results is microscopic, according to the data.
...
Reading scores increase by 4%

After implementing a new reading program, reading test scores are up by more than 3 points among third-grade students in the district. 

### Reflection Question 6:
What metrics or outcomes beyond test scores might be important to track for assessing reading performance?

You could assess time spent reading, comprehension, time spent rereading, and the complexity of the books that students choose to read.
