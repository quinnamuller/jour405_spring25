...
```{r}
#find the average maltreatment value per county from 2012 to 2017 (got stuck and had some help from chatgpt)
child_maltreatment <- child_maltreatment %>%
  group_by(jurisdiction) %>%
  mutate(child_maltreatment = mean(value, na.rm = TRUE)) %>%
  mutate(jurisdiction)

#create new simplified chart with this data (got stuck and had some help from chatgpt)
  child_maltreatment <- child_maltreatment %>%
  group_by(jurisdiction) %>%
